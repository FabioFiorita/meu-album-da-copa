...
export function AlbumTab({ session }: Props) {
  ...
  useLayoutEffect(() => {
    const scroller = topRef.current?.closest<HTMLElement>(
  ...
  async function addSticker(key: string, label: string) {
    try {
      await addCopies({
        code: session.code,
        writeKey: session.writeKey,
  ...
  async function removeSticker(key: string, label: string, count: number) {
    if (count <= 0) {
      toast.warning(`${label} ainda não está no álbum.`);
      return;
    }
...
function SectionDetail({
  ...
  onBack: () => void;
  onAddSticker: (key: string, label: string) => Promise<void>;
  onRemoveSticker: (key: string, label: string, count: number) => Promise<void>;
}) {
        ...
        <div className="relative z-10 flex flex-col gap-3">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              aria-label={
                removeMode
                  ? "Voltar ao fluxo padrão"
                  : "Ativar modo remover"
              }
              onClick={() => setRemoveMode((current) => !current)}
              className={cn(
                "flex size-8 shrink-0 items-center justify-center rounded-xl border outline-none transition-colors focus-visible:ring-2 focus-visible:ring-white/35",
                removeMode
                  ? "border-[#d6b45d] bg-[#171717]/95 text-[#f4d77c]"
                  : "border-white/25 bg-black/32 text-white hover:bg-white/12",
              )}
            >
              {removeMode ? (
          ...
          {removeMode && (
            <div className="album-remove-mode-message rounded-xl border border-[#d6b45d]/55 bg-[#171717]/92 px-3 py-2 text-center text-[12px] font-black leading-none tracking-normal text-[#f4d77c] shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
              Clique na figurinha para remover
            </div>
          )}
            ...
            {section.stickers.map((st) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    const label = `${section.id}:${st.displayNumber}`;
                    if (removeMode) {
                      void onRemoveSticker(st.key, label, c);
                      return;
                    }
                    void onAddSticker(st.key, label);
                  }}
                  aria-label={`${section.id} ${st.displayNumber} ${
                    removeMode ? "remover figurinha" : "adicionar figurinha"
                  }`}
                  ...
                  className={cn(
                    ...
                    !owned && "opacity-95",
                    owned &&
                      "ring-1 ring-[#d6b45d]/75 shadow-[0_0_0_1px_rgba(0,0,0,0.58),0_4px_12px_rgba(0,0,0,0.32),inset_0_1px_0_rgba(255,255,255,0.18)]",
                  )}
                >
                  ...
                  {owned && (
                    <span className="absolute right-1 top-1 z-20 flex size-5 items-center justify-center rounded-full border border-[#d6b45d] bg-[#171717]/95 text-[#f4d77c] shadow-[0_0_0_1px_rgba(0,0,0,0.72),0_3px_8px_rgba(0,0,0,0.42)]">
